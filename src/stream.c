...
static uv_stream_t* luv_check_stream(lua_State* L, int index) {
  ...
  int isStream = lua_getmetatable(L, index < 0 ? index - 1 : index);
  if (!isStream)
    goto fail;
  lua_rawget(L, -2);
  ...
  uv_stream_t* handle = *(uv_stream_t**)udata;
  if (!handle->data)
    goto fail;
  return handle;
...
static int luv_listen(lua_State* L) {
  ...
  int ret = uv_listen(handle, backlog, luv_connection_cb);
...
static int luv_read_start(lua_State* L) {
  ...
  int ret = uv_read_start(handle, luv_alloc_cb, luv_read_cb);
  return luv_result(L, ret);
}
...
static int luv_write(lua_State* L) {
  ...
  uv_write_t* req = (uv_write_t*)lua_newuserdata(L, uv_req_size(UV_WRITE));
  req->data = (luv_req_t*)luv_setup_req(L, ctx, ref);
  size_t count;
  uv_buf_t* bufs = luv_check_bufs(L, 2, &count, (luv_req_t*)req->data);
  int ret = uv_write(req, handle, bufs, count, luv_write_cb);
  free(bufs);
  if (ret < 0) {
    luv_cleanup_req(L, (luv_req_t*)req->data);
    lua_pop(L, 1);
...
static int luv_write2(lua_State* L) {
  ...
  size_t count;
  uv_buf_t* bufs = luv_check_bufs(L, 2, &count, (luv_req_t*)req->data);
  int ret = uv_write2(req, handle, bufs, count, send_handle, luv_write_cb);
  free(bufs);
  if (ret < 0) {
    luv_cleanup_req(L, (luv_req_t*)req->data);
    lua_pop(L, 1);
...
static int luv_try_write(lua_State* L) {
  ...
  int err_or_num_bytes = uv_try_write(handle, bufs, count);
  free(bufs);
...
static int luv_try_write2(lua_State* L) {
  ...
  uv_stream_t* send_handle = luv_check_stream(L, 3);
  uv_buf_t* bufs = luv_check_bufs_noref(L, 2, &count);
  int err_or_num_bytes = uv_try_write2(handle, bufs, count, send_handle);
  free(bufs);
  if (err_or_num_bytes < 0)
    return luv_error(L, err_or_num_bytes);
  lua_pushinteger(L, err_or_num_bytes);
...
static int luv_stream_set_blocking(lua_State* L) {
  ...
  int ret = uv_stream_set_blocking(handle, blocking);
  return luv_result(L, ret);
}
