...
static int luv_handle_gc(lua_State *L) {
  ...
  // if the handle is not yet closed, we must close it before allowing lua to collect the userdata
  // we do this regardless of whether the handle is already closing or not to ensure we override any
  // previous close callbacks so we can immediately release the userdata
  uv_close(handle, luv_handle_gc_cb);

  // we must "permanently" resurrect the userdata by holding a reference to it while we wait for the close
  lua_pushvalue(L, 1);
  lhandle->ref = luaL_ref(L, LUA_REGISTRYINDEX);
