...
static int luv_push_constant_table(lua_State* const L) {
  const int tbl_size = (sizeof(constants) / sizeof(struct constant)) +
                       (sizeof(address_families) / sizeof(struct constant)) +
...
static const char* luv_addrfamily_int2str(const int num) {
  for (const struct constant* val = address_families; val->name != NULL; val++)
    if (num == val->value)
      return val->name;
...
static const char* luv_sockdomain_int2str(const int num) {
  for (const struct constant* val = socket_domains; val->name != NULL; val++)
    if (num == val->value)
      return val->name;
...
static const char* luv_signal_int2str(const int num) {
  for (const struct constant* val = signals; val->name != NULL; val++)
    if (num == val->value)
      return val->name;
