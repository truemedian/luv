...
static void luv_async_cb(uv_async_t* handle) {
  ...
  int n = luv_thread_arg_push(L, (luv_thread_args_t*)data->extra, LUVF_THREAD_SIDE_MAIN);
  luv_call_callback(L, data, LUV_ASYNC, n);
  luv_thread_arg_clear(L, (luv_thread_args_t*)data->extra, LUVF_THREAD_SIDE_MAIN);
}
...
static int luv_new_async(lua_State* L) {
  ...
  data->extra = (luv_thread_args_t*)malloc(sizeof(luv_thread_args_t));
  data->extra_gc = free;
  memset(data->extra, 0, sizeof(luv_thread_args_t));
  handle->data = data;
...
static int luv_async_send(lua_State* L) {
  ...
  luv_thread_args_t* arg = (luv_thread_args_t *)((luv_handle_t*) handle->data)->extra;
