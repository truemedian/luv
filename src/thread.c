  ...
  while (i < args->argc) {
    ...
    switch (arg->type) {
      ...
      case LUA_TUSERDATA:
        ...
        break;
      default:
        fprintf(stderr, "Error: thread arg not support type %s at %d", lua_typename(L, arg->type), i + 1);
    }
    i++;
  };
  return i;
...
static int luv_thread_arg_error(lua_State* L) {
  ...
  lua_pop(L, 2);
  return luaL_error(L, "Error: thread arg not support type '%s' at %d", lua_typename(L, type), pos);
}
...
static int luv_thread_dumped(lua_State* L, int idx) {
  if (lua_isstring(L, idx)) {
    ...
    lua_pushvalue(L, idx);
    int ret = lua_dump(L, thread_dump, &state, 1);
    if (ret == 0) {
...
static int luv_new_thread(lua_State* L) {
  ...
  luv_thread_t* thread = (luv_thread_t*)lua_newuserdata(L, sizeof(*thread));
  ...
  thread->ref = LUA_NOREF;
  thread->L = L;
  int ret = uv_async_init(ctx->loop, &thread->notify, luv_thread_exit_cb);
  if (ret < 0)
...
static int luv_thread_getaffinity(lua_State* L) {
  ...
  if (mask_size < default_mask_size) {
    return luaL_argerror(
      L, 2, lua_pushfstring(L, "cpumask size must be >= %d (from cpumask_size()), got %d", default_mask_size, mask_size)
    );
  }
...
static void luv_thread_init(lua_State* L) {
  ...
  if (acquire_vm_cb == NULL)
    acquire_vm_cb = luv_thread_acquire_vm;
  if (release_vm_cb == NULL)
