...
/* [-0, +1] creates registers and pushes a uv handle metatable pre-filled with all handle and provided methods */
static int luv_handle_metatable(lua_State *L, const char *name, const luaL_Reg *methods);

/* [-0, +1] create a luv_handle_t with an attached uv handle and extra data */
static luv_handle_t *luv_handle_create(lua_State *L, uv_handle_type type, const luv_ctx_t *ctx, size_t extra_sz);

/* [-0, +0] remove all references this handle is holding to allow it to be garbage collected */
static void luv_handle_unref(lua_State *L, luv_handle_t *handle);

/* [-0, +1] push the userdata associated with this handle to the stack */
static void luv_handle_push(lua_State *L, luv_handle_t *handle);

/* [-0, +0] ensure this handle is still valid and throw an error otherwise */
static void luv_handle_check_valid(lua_State *L, luv_handle_t *handle);

/* [-0, +0] set a callback for a specific event on this handle to the callable at `index` */
static void luv_handle_set_callback(lua_State *L, enum luv_callback_id what, luv_handle_t *handle, int index);

/* [-0, +0] unset a callback for a specific event on this handle, usually after failure to start the handle */
static void luv_handle_unset_callback(lua_State *L, enum luv_callback_id what, luv_handle_t *handle);

/* [-0, +0] call the callback registered to `what` with the top `nargs` of the stack or pop `nargs` off the stack */
static void luv_handle_send_callback(lua_State *L, enum luv_callback_id what, luv_handle_t *handle, int nargs);
