...
static void luv_pushaddrinfo(lua_State* L, struct addrinfo* res) {
  ...
  int i = 0;
  lua_newtable(L);
  for (struct addrinfo* curr = res; curr; curr = curr->ai_next) {
...
static int luv_getnameinfo(lua_State* L) {
  ...
  uv_getnameinfo_t* req = (uv_getnameinfo_t*)lua_newuserdata(L, uv_req_size(UV_GETNAMEINFO));
  req->data = luv_setup_req(L, ctx, ref);

  int ret = uv_getnameinfo(ctx->loop, req, ref == LUA_NOREF ? NULL : luv_getnameinfo_cb, (struct sockaddr*)&addr, 0);
  if (ret < 0) {
    luv_cleanup_req(L, (luv_req_t*)req->data);
    lua_pop(L, 1);
    return luv_error(L, ret);
