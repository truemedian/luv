...
static int luv_work_cb(lua_State* L) {
  ...
  if (lua_isfunction(L, -1)) {
    ...
    if (i >= 0) {
      ...
      i = luv_thread_arg_set(L, &work->rets, top + 1, lua_gettop(L), LUVF_THREAD_MODE_ASYNC | LUVF_THREAD_SIDE_CHILD);
      if (i < 0) {
...
static void luv_after_work_cb(uv_work_t* req, int status) {
  ...
  int narg = luv_thread_arg_push(L, &work->rets, LUVF_THREAD_SIDE_MAIN);
  lctx->cb_pcall(L, narg, 0, 0);
...
static int luv_new_work(lua_State* L) {
  ...
  luv_work_ctx_t* ctx = (luv_work_ctx_t*)lua_newuserdata(L, sizeof(*ctx));
...
static const luaL_Reg luv_work_ctx_methods[] = {{"queue", luv_queue_work}, {NULL, NULL}};
...
static void luv_work_init(lua_State* L) {
  ...
  if (val != NULL)
    nvms = (unsigned int)strtoul(val, NULL, 10);
  if (nvms == 0)
    nvms = 1;
